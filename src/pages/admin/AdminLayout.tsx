...
import { Sun, Moon } from "@phosphor-icons/react"; // Add these imports
...
const AdminLayout = ({ children, requireAuth = true, showSidebar = true }: AdminLayoutProps) => {
  ...
  // Open sidebar when authenticated and not mobile
  useEffect(() => {
    if (!isLoading && isAdminAuthenticated && !isMobileView) {
      setSidebarOpen(true);
    }
  }, [isAdminAuthenticated, isLoading, isMobileView]);
  ...
  // Prevent unnecessary reloads on admin dashboard route
  useEffect(() => {
    // Only run this effect on the client
    if (typeof window !== "undefined") {
      // Remove any forced reload on /admin/dashboard
      if (
        ...
        // Do nothing, allow SPA navigation
      }
    }
  }, []);
    ...
    <PageTransition>
      <div className="flex min-h-screen bg-background">
        {showSidebar && (
          <>
            {/* Mobile overlay */}
            {isMobileView && sidebarOpen && (isAdminAuthenticated || isLoading) && (
              <div
